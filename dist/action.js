...
async function action(octokit, pr) {
    ...
    let detectedReverts = new UpstreamRelatedCommits({ heading: 'Reverts', tableHeader: 'revert' }, upstreamOwner, upstreamRepo);
    let detectedFollowUps = new UpstreamRelatedCommits({ heading: 'Follow-ups', tableHeader: 'follow-up' }, upstreamOwner, upstreamRepo);
    let detectedMentions = new UpstreamRelatedCommits({ heading: 'Commit mentions', tableHeader: 'mention' }, upstreamOwner, upstreamRepo);
    // clone upstream and downstream git repo
    const upstreamGit = new Git(upstreamOwner, upstreamRepo, 'abc_UpstreamRepo_cba');
    ...
    if (detectedReverts.isRelatedCommitDetected()) {
        ...
        if (!isWaived) {
            ...
            statusSummary.push('🔴 Some revert commits for this Pull Request were detected in upstream');
        }
        else {
    ...
    if (detectedFollowUps.isRelatedCommitDetected()) {
        ...
        if (!isWaived) {
            ...
            statusSummary.push('🔴 Some follow-up commits for this Pull Request were detected in upstream');
        }
        else {
    ...
    if (detectedMentions.isRelatedCommitDetected()) {
        ...
        if (!isWaived) {
            ...
            statusSummary.push('🔴 Some mentions of commits from this Pull Request were detected in upstream');
        }
        else {
